...
def _tolov_yaratish_sync(tolov_id, uid, ism, fayl_nomi, fayl_bayt, yaratilgan_vaqt):
    conn = _ulanish()
    conn.execute(
        """INSERT INTO tolovlar (id, uid, ism, fayl_nomi, fayl_bayt, holat, yaratilgan_vaqt)
           VALUES (?,?,?,?,?, 'kutilmoqda', ?)""",
        (tolov_id, uid, ism, fayl_nomi, fayl_bayt, yaratilgan_vaqt))
    conn.commit()
    conn.close()


async def tolov_yaratish(tolov_id, uid, ism, fayl_nomi, fayl_bayt, yaratilgan_vaqt):
    """Yangi kutilayotgan to'lov (va unga tegishli fayl) yozadi."""
    await _ish(_tolov_yaratish_sync, tolov_id, uid, ism, fayl_nomi, fayl_bayt, yaratilgan_vaqt)
...
async def tolov_holatini_yangilash(tolov_id, holat):
    """holat: 'kutilmoqda' | 'chek_yuborildi' | 'tasdiqlangan' | 'rad_etilgan'."""
